...
app.get('/', (req, res) => {
    mysql.query('SELECT * FROM gofiq', (err, results) => {
        if (err) throw err;
...
app.post('/add', upload.single('foto'), (req, res) => {
    const { nama, alamat, notlp } = req.body;
    const foto = req.file ? '/uploads/' + req.file.filename : null;

    mysql.query('INSERT INTO gofig (nama, alamat, notlp ) VALUES (?, ?, ?)', [nama, alamat, notlp ], (err, result) => {
        if (err) throw err;
        res.redirect('/');
    });
});
...
app.get('/edit/:id', (req, res) => {
    ...
    mysql.query('SELECT * FROM gofig WHERE id = ?', [id], (err, results) => {
        if (err) throw err;
...
app.post('/edit/:id', upload.single('foto'), (req, res) => {
    ...
    mysql.query('UPDATE gofiq SET nama = ?, alamat = ?, notlp = ? WHERE id = ?', [nama, alamat, notlp, id], (err, result) => {
        if (err) throw err;
        res.redirect('/');
    });
});
...
app.get('/delete/:id', (req, res) => {
    const id = req.params.id;
    mysql.query('DELETE FROM gofiq WHERE id = ?', [id], (err, result) => {
        if (err) throw err;
        res.redirect('/');
    });
});
